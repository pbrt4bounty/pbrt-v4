    ...
    for (auto iter = args.begin(); iter != args.end(); ++iter) {
        ...
        if (ParseArg(&iter, args.end(), "cropwindow", &cropWindow, onError)) {
            ...
            ParseArg(&iter, args.end(), "disable-ms", &options.disableMultipleScattering,
                     onError) ||
            ParseArg(&iter, args.end(), "disable-ss-sur",
                     &options.disableSingleScatteringSurface, onError) ||
            ParseArg(&iter, args.end(), "disable-ss-vol",
                     &options.disableSingleScatteringVolume, onError) ||
