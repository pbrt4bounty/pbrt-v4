...
std::unique_ptr<PathIntegrator> PathIntegrator::Create(
    ...
    return std::make_unique<PathIntegrator>(maxDepth, minRRDepth, camera, sampler,
...
std::unique_ptr<VolPathIntegrator> VolPathIntegrator::Create(
    ...
    std::string lightStrategy = parameters.GetOneString("lightsampler", "bvh");
    bool regularize = parameters.GetOneBool("regularize", false);
    return std::make_unique<VolPathIntegrator>(maxDepth, minRRDepth, camera, sampler,
                                               aggregate, lights, lightStrategy,
                                               regularize);
}
...
void SPPMIntegrator::Render() {
    ...
    if (Options->interactive && Options->displayServer.empty()) {
        Float moveScale = Length(pixelBounds.Diagonal()) / 1000.f;
    ...
    for (int iter = 0; iter < nIterations; ++iter) {
        ...
        if (gui) {
            ...
            int percentage = std::min(iter, nIterations);
