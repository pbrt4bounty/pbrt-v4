...
Procedural::Procedural(const std::string &filepath, const std::string &namedMaterial,
...
std::string Procedural::constructPbrtMaterialBlock() {
    ...
    if (!namedMaterial.empty()) {
        ...
        if (!texture.empty()) {
            block +=
                "Texture \"" + namedMaterial + "_diffuse\" \"spectrum\" \"imagemap\"\n";
            block += "    \"string filename\" [ \"" + texture + "\" ]\n\n";
        }
        if (!bumpMap.empty()) {
            block +=
                "Texture \"" + namedMaterial + "_roughness\" \"float\" \"imagemap\"\n";
